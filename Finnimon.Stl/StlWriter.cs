...
public static class StlWriter
{
    ...
    private static bool WriteBinary(Stl stl, Stream stream, bool leaveOpen)
    {
        ...
        if (combinedHeader.StartsWith("solid", StringComparison.OrdinalIgnoreCase))
            combinedHeader = "_" + combinedHeader;
        ...
        {
            var n = facet.Triangle.Normal;
            ReadOnlySpan<float> floats = n;
            foreach(var f in floats) binary.Write(f);
            floats = facet.Triangle;
            foreach(var f in floats) binary.Write(f);
